...
class IssueModel extends AbstractModel
{
    ...
    public function upsert($input)
    {
        ...
        if ($input['user_id']) {
            foreach ($input['user_id'] as $detail) {
        ...
        if ($id) {
            ...
            $task->updated_by = OFFICE::getCurrentUserId();
            $issueId = $task->id;
        ...
        $userIds = (array_pluck($input['user_id'], 'id'));
        if (!empty($userIds)) {
            $task->issueUser()->sync($userIds);
        }
        return $task;
    }
...
//            }
//            if (!empty($input['status'])) {
//                $query = $query->where($issuesTable . '.status', "{$input['status']}");
//            }
//            if (!empty($input['priority'])) {
//                $query = $query->where($issuesTable . '.priority', "{$input['priority']}");
//            }
//            $query = $query->orderBY($issuesTable . '.id', 'DESC');
//        }
//
...
//    }
    ...
    public function search($input = [], $with = [], $limit = null)
    {
        ...
        if ($roleCurrent == USER_ROLE_ADMIN && $isSuper == true) {
            if (!empty($input['module_id'])) {
                $moduleId = $input['module_id'];
                $query = $query->whereHas('moduleCategory.module', function ($q) use ($moduleId) {
                    $q->where('id', $moduleId);
                });
            }
        } elseif ($roleCurrent == USER_ROLE_ADMIN && $isSuper == false) {
            $query = $query->whereHas('moduleCategory.module', function ($q) use ($moduleIds, $user_id) {
                $q->whereIn('id', $moduleIds)
                    ->orWhere('created_by', $user_id);
            });
            $query = $query->orWhere('created_by', $user_id);
        } else {
            $query = $query->whereHas('moduleCategory.module', function ($q) use ($moduleIds) {
                $q->whereIn('id', "$moduleIds");
            });
        }
        if (!empty($input['module_code'])) {
            $moduleCode = $input['module_code'];
            $query = $query->whereHas('moduleCategory.module', function ($q) use ($moduleCode) {
                $q->where('code', 'like', "%$moduleCode%");
            });
        }
        if (!empty($input['module_name'])) {
            $moduleName = $input['module_name'];
            $query = $query->whereHas('moduleCategory.module', function ($q) use ($moduleName) {
                $q->where('name', 'like', "%$moduleName%");
            });
        }
        if (!empty($input['full_name'])) {
            $fullName = $input['full_name'];
            $query = $query->whereHas('issueHasUser.user.profile', function ($q) use ($fullName) {
    ...
    public function searchMine($input = [], $with = [], $limit = null)
    {
        $query = $this->make($with);
        $this->sortBuilder($query, $input);
        $idMine = OFFICE::getCurrentUserId();
        $issues = new IssueModel();
        $issuesTable = $issues->getTable();
        $modules = new ModuleModel();
        $modulesTable = $modules->getTable();
    ...
    public function searchUserDaiLy($input = [], $with = [], $limit = null)
    {
        $query = $this->make($with);
        $this->sortBuilder($query, $input);
