...
class RoleController extends BaseController
{
     ...
     * @param HealthBookRecordTransformer $roleTransformer
    ...
    public function store(Request $request, RoleValidator $roleValidator, RoleTransformer $roleTransformer)
    {
        ...
        try {
            ...
            Log::create($this->model->getTable(), $role->code);
        } catch (\Exception $ex) {
            DB::rollBack();
            $response = OFFICE_Error::handle($ex);
            return $this->response->errorBadRequest($response['message']);
        }
        return ["id"=>$role->id, 'status' => Message::get("R001", $role->id), 'message' =>  Message::get("R001", $role->code)];
    }
    ...
    public function update($id, Request $request, RoleUpdateValidator $roleUpdateValidator, RoleTransformer $roleTransformer)
    {
        ...
        try {
            DB::beginTransaction();
            $role = $this->model->upsert($input);
            Log::update($this->model->getTable(), $role->code);
            DB::commit();
        } catch (\Exception $ex) {
            DB::rollBack();
            $response = OFFICE_Error::handle($ex);
        ...
        return ["id"=>$role->id, 'status' =>Message::get("R002", $role->id), 'message' =>  Message::get("R002", $role->code)];
    }
    ...
    public function delete($id)
    {
        try {
            ...
            // 1. Delete Role
            $role->delete();
            Log::delete($this->model->getTable(), $role->code);
            DB::commit();
        } catch (\Exception $ex) {
            $response = OFFICE_Error::handle($ex);
            return $this->response->errorBadRequest($response['message']);
        }
        return ['status' => Message::get("R003", $role->id), 'message' =>  Message::get("R003", $role->code)];
    }
